...
const KhaltiSuccess = () => {
    ...
    useEffect(() => {
        ...
        const callbackStatus = params.get('status'); // Khalti returns Completed/User canceled/etc.

        if (!pidx) {
            setStatus('failed');
            setMessage('Missing payment reference (pidx) from Khalti.');
            return;
        }
        if (!auth?.token) {
            setStatus('failed');
        ...
        const verify = async () => {
            try {
                const res = await axios.post(
                    `${BASE_URL}/api/v1/payment/khalti/verify`,
                    { pidx },
                ...
                if (res?.data?.success) {
                    ...
                    setMessage(
                        res?.data?.message ||
                            (callbackStatus ? `Khalti reported: ${callbackStatus}` : 'Payment verification failed')
                    );
                }
            } catch (e) {
                if (cancelled) return;
                ...
                <div className="bg-tailor-darker border border-tailor-gold/30 rounded-2xl p-10 text-center shadow-2xl">
                    {status === 'verifying' && (
                        <>
                            ...
                            <p className="text-gray-400">Confirming your transaction with Khalti.</p>
                        </>
                    )}
                        ...
                        <>
                            ...
                            {order && (
                                <div className="bg-tailor-black/50 border border-tailor-gold/20 rounded-xl p-6 text-left mb-8 space-y-2 text-sm">
                                    ...
                                    <div className="flex justify-between"><span className="text-gray-500">Method</span><span className="text-white capitalize">{order.paymentMethod}</span></div>
                                    <div className="flex justify-between"><span className="text-gray-500">Transaction ID</span><span className="text-white font-mono text-xs break-all">{order.transactionId || order.pidx}</span></div>
                    ...
                    {status === 'failed' && (
                        <>
                            ...
                            <h1 className="text-3xl font-serif text-white mb-3">Payment Not Completed</h1>
                            <p className="text-gray-300 mb-8">{message || 'We could not verify your Khalti payment.'}</p>
                            <div className="flex flex-col sm:flex-row gap-3 justify-center">
