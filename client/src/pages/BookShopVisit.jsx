...
const BookShopVisit = () => {
    ...
    const [form, setForm] = useState({
        ...
        address: 'Physical Boutique', // Placeholder since it's required by some validation maybe, but I made it optional in model
        city: 'Kathmandu',
    });
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            if (res?.data?.success) {
                toast.success('Shop appointment booked successfully!');
                navigate('/');
            } else {
                toast.error(res?.data?.message || 'Failed to book shop visit');
            }
        } catch (error) {
            console.log(error);
                ...
                <div className="max-w-3xl mx-auto" data-aos="fade-up">
                    <div className="text-center mb-10">
                        <h1 className="text-4xl md:text-5xl font-serif text-white mb-4">
                            Visit Our <span className="text-tailor-gold">Boutique</span>
                        </h1>
                        <p className="text-gray-400 text-lg">
                            Schedule a visit to our flagship showroom for a premium measurement experience and fabric consultation.
                        </p>
                    </div>
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                            ...
                            <div>
                                <label className="block text-sm text-tailor-gold mb-2">Location Preference</label>
                                <select 
                                    name="city" 
                                    value={form.city} 
                                    onChange={handleChange}
                                    className={inputClass}
                                >
                                    <option value="Kathmandu (New Baneshwor)">New Baneshwor, Kathmandu</option>
                                    <option value="Bhaktapur (Kamalbinayak)">Kamalbinayak, Bhaktapur</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea
                                name="notes"
                                placeholder="Tell us if you're looking for something specific..."
                                value={form.notes}
                                onChange={handleChange}
                                rows="4"
                                className={inputClass}
                        ...
                        <div className="mt-8 pt-8 border-t border-white/10 text-center">
                            <h4 className="text-white font-medium mb-4">Our Locations:</h4>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                                <div className="text-gray-400">
                                    <p className="text-tailor-gold font-bold">New Baneshwor</p>
                                    <p>Main Road, Near Civil Hospital</p>
                                    <p>Kathmandu</p>
                                </div>
                                <div className="text-gray-400">
                                    <p className="text-tailor-gold font-bold">Kamalbinayak</p>
                                    <p>Opposite to Bus Park</p>
                                    <p>Bhaktapur</p>
                                </div>
                            </div>
                        </div>
